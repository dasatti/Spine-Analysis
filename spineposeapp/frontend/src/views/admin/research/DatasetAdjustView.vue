...
import {
  ...
  saveDatasetManualLabels,
} from '../../../api/client'
import {
  emptyManualLabelForm,
  MANUAL_LABEL_GROUPS,
  MANUAL_LABEL_OPTIONS,
  manualLabelsFromKeypoints,
  manualLabelsPayload,
} from '../../../utils/datasetManualLabels'
...
const manualLabelsForm = ref(emptyManualLabelForm())
const labelsSaving = ref(false)
const labelsError = ref('')
const labelsSaved = ref(false)

const manualLabelsMeta = computed(() => item.value?.keypoints?.manual_labels || null)
...
async function saveManualLabels() {
  ...
  try {
    const { data } = await saveDatasetManualLabels(
      item.value.id,
      manualLabelsPayload(manualLabelsForm.value)
    )
    item.value = data
    manualLabelsForm.value = manualLabelsFromKeypoints(data.keypoints)
    labelsSaved.value = true
  } catch (e) {
    labelsError.value =
      e.response?.data?.message || e.response?.data?.detail?.message || 'Failed to save labels.'
  } finally {
...
<template>
  ...
  >
    ...
    <div v-else-if="item" class="p-gutter lg:px-margin-desktop space-y-stack-lg pb-margin-desktop">
      ...
      <div v-if="item.status === 'ready'" class="space-y-stack-lg">
        <div class="grid grid-cols-12 gap-gutter items-end">
          <div class="col-span-12 lg:col-span-5 flex flex-wrap items-center justify-between gap-3">
          ...
          <div class="col-span-12 lg:col-span-7">
            <h3 class="font-label-caps text-primary">Detected Keypoints</h3>
          </div>
        </div>
        ...
        <div class="grid grid-cols-12 gap-gutter items-start">
          ...
          <div class="col-span-12 lg:col-span-7">
            <div class="bg-surface-container border border-outline-variant h-[560px] flex flex-col">
        ...
        <div class="grid grid-cols-12 gap-gutter items-start">
          <section class="col-span-12 lg:col-span-5 space-y-4">
            <div>
              ...
              <div v-else class="bg-surface-container border border-outline-variant max-h-[525px] overflow-y-auto">
                <table class="w-full text-sm">
          ...
          <section class="col-span-12 lg:col-span-7 space-y-4">
            <form
              class="bg-surface-container border border-outline-variant p-4"
              @submit.prevent="saveManualLabels"
            >
            <div class="flex flex-wrap items-center justify-between gap-2 mb-4">
              <div>
                <h3 class="font-label-caps text-primary text-xs">Manual Clinical Labels</h3>
                <p
                  v-if="manualLabelsMeta?.labeled_at"
                  class="text-xs text-on-surface-variant mt-0.5"
                >
                  Saved {{ formatDate(manualLabelsMeta.labeled_at) }}
                </p>
              </div>
              <div class="flex items-center gap-2">
                <span v-if="labelsSaved" class="text-primary text-xs font-label-caps">Saved</span>
                <button
                  class="px-3 py-1.5 bg-primary text-on-primary font-label-caps text-xs font-bold hover:opacity-90 disabled:opacity-50"
                  type="submit"
            ...
            <p v-if="labelsError" class="text-error text-sm mb-2">{{ labelsError }}</p>

            <div class="space-y-4">
              <div v-for="group in MANUAL_LABEL_GROUPS" :key="group.title">
                <p class="font-label-caps text-xs text-on-surface-variant mb-2">
                  {{ group.title }}
                </p>
                <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-x-4 gap-y-2">
                  ...
                  >
                    <span
                      class="text-sm text-on-surface truncate flex-1 leading-snug"
                      :title="field.label"
                      >{{ field.label }}</span
                    >
                    <select
                      v-model="manualLabelsForm[field.key]"
                      class="w-[5rem] shrink-0 bg-background border border-outline-variant text-sm text-on-surface px-1.5 py-1 outline-none focus:border-primary-container"
                    >
                      <option v-for="opt in MANUAL_LABEL_OPTIONS" :key="opt.value" :value="opt.value">
                        {{ opt.label }}
                      </option>
                    </select>
                  </label>
